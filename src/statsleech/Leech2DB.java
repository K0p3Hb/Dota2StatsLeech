...
public class Leech2DB {
  ...
  public void initialize() {
    ...
    try {
      ...
      statement
          .execute("create table if not exists items (item_id int primary key, item_name varchar, image_url varchar)");
      // System.out.println("Базa вещей создана");
      // создаём таблицу героев
      statement
          .execute("create table if not exists heroes (hero_id int primary key, hero_name varchar, image_url varchar)");
      // System.out.println("Базa героев создана");
      // создаём таблицу матчей
      statement.execute("create table if not exists matches (match_id bigint primary key, duration int,"
          + " game_mode int," + " radiant_win boolean, top_hero_damage bigint references accounts(account_id),"
      ...
      statement.execute("create table if not exists all_player_slots"
          + " (match_id bigint references matches(match_id)," + " hero_id int references heroes(hero_id),"
          + " account_id bigint references accounts(account_id)," + " slot_number int," + " kills int," + " deaths int,"
          + " assists int," + " item_0 int references items(item_id)," + " item_1 int references items(item_id),"
  ...
  public void updateMatchesAndSlots(MatchDetail match) {
    ...
    try {
      ...
      statement = connect.prepareStatement(
          "insert into matches" + " (match_id," + " duration," + " game_mode," + " radiant_win," + "top_hero_damage,"
      ...
      long tmpKill = 0, tmpLast = 0, tmpTower = 0, tmpHeroDamage = 0, tmp = 1, tmpKill1 = 0, tmpTower1 = 0,
          ...
          topKiller1 = 0, radiantHero = 0, direHero = 0, radiantTower = 1, direTower = 1;
   ...
   * 
   * statement.setLong(i++, player.getHeroDamageDealt()); statement.setLong(i++,
   * player.getTowerDamageDealt()); statement.setInt(i++, player.getLastHits());
   * 
