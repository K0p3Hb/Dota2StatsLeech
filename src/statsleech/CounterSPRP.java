...
public class CounterSPRP {
  ...
  public void countFightSP(MatchDetail match, int slot, JdbcConnectionSource connect, float killer) {
    ...
    spDamage = (float) ((((currentDamage * 100f) / allDamage) - ((killer * 100f) / allDamage)) * 0.3) + 3;
    spActivity = (float) (((currentActivity * 100f) / allActivity) * 0.025) + 2;
  ...
  public void countSurvivalSP(MatchDetail match, int slot, JdbcConnectionSource connect, float deaths, float heal) {
    ...
    spDeath = (float) (((((deaths / (match.getDurationOfMatch() / 60f)) * 100f) / allDeath)
        - ((currentDeath * 100f) / allDeath)) * 0.3 + 3);
    if (spDeath < 0)
      spDeath = 0.01f;
